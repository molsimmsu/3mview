 ...
 *	$Revision: 1.337 $	$Date: 2011/10/17 18:36:34 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
    ...
    switch (atomRep->method()) {
      ...
      case AtomRep::QUICKSURF:
        draw_quicksurf(framepos,
          atomRep->get_data(AtomRep::BONDRES),      // quality level
...
#else
  cmdPickPointArray.putdata(atomSel->num_atoms, atomSel->selected, 
                            &atomSel->on[0], pos, cmdList);
#endif
}
...
void DrawMolItem::draw_residue_beads(float * framepos, int sres, float radscale) {
  ...
  for (resid=0; resid<numres; resid++) {
    ...
    centers.append(com[0]);
    centers.append(com[1]);
...
#else
  if (atomSel->selected > 0) {
    float *centers = new float[atomSel->selected * 3];
    float *colors  = new float[atomSel->selected * 3];
    const float *fp = framepos + (atomSel->firstsel * 3); 
    int i, ind;
    for (ind=0,i=atomSel->firstsel; i <= atomSel->lastsel; i++) {
      // draw a sphere for each selected atom
      if (atomSel->on[i]) {
        centers[ind    ] = fp[0];
        centers[ind + 1] = fp[1];
        ...
        const float *cp = scene->color_value(atomColor->color[i]);
        colors[ind    ] = cp[0];
        colors[ind + 1] = cp[1];
        colors[ind + 2] = cp[2];
        ind += 3;
  ...
  for (loop=-1; loop<num; loop++) { // go through the array
    ...
    if ((idx[loop  ] >=0 && atomSel->on[idx[loop  ]]) ||
      ...
      if (idx[loop] >= 0 &&          // this is a real atom
        ...
        int pidx = 3 * loop;
        pickpointcoords.append(coords[pidx    ]);
        pickpointcoords.append(coords[pidx + 1]);
        pickpointcoords.append(coords[pidx + 2]);
    ...
    if (!a2->bonded(p->ind1)) {
	  ...
	  vec_sub(Htoacceptor,coor1,hydrogen);
	    ...
	    cmdLine.putdata(coor1,hydrogen, cmdList); // draw line
            ...
            pickpointcoords.append(framepos[pidx    ]);
            pickpointcoords.append(framepos[pidx + 1]);
            pickpointcoords.append(framepos[pidx + 2]);
            ...
            pickpointindices.append(a2->bondTo[k]);
            pickpointindices.append(p->ind1);
          }
        }
      }
      for (k=0; k < b1; k++){
	    ...
	    cmdLine.putdata(hydrogen,coor2, cmdList); // draw line
            ...
            pickpointindices.append(a1->bondTo[k]);
            pickpointindices.append(p->ind2);
          }
        }
      }
    }
       ...
       float *start_coord, float *end_coord)
{
  ...
  for (int i=0; i<x.num(); i++) {
    ...
    if (atom_on[i] >= 0) {
      // indicate this atom can be picked
      pickpointcoords.append(x[i]);
      pickpointcoords.append(y[i]);
      pickpointcoords.append(z[i]);
       ...
       int *color, float ribbon_width, float *start_coord, float *end_coord)
{
  ...
  if (atom_on[num-1] >= 0) {
    // indicate this atom can be picked
    pickpointcoords.append(x[num-1]);
    pickpointcoords.append(y[num-1]);
    pickpointcoords.append(z[num-1]);
